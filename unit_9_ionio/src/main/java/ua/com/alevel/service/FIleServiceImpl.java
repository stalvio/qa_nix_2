...
public class FIleServiceImpl implements FileService {
    ...
    public void create(FileModel fileModel) {
        ...
        switch(fileModel.getFileType()) {
            case INPUT: {
                file = new File(BASE_PATH + fileModel.getFileType().getFileName() + ".txt");
            } break;
            case OUTPUT: {
                String inputContent = read(BASE_PATH + "input.txt");
                String outputContent = getStatistic(inputContent);
                fileModel.setContent(outputContent);
                fileModel.setFileType(FileType.OUTPUT);
                file = new File(BASE_PATH + fileModel.getFileName());
            } break;
        }
    ...
    private Map<String, Integer> getNumberOfDuplicates(String[] words) {
        ...
        for(String str: wordsSet) {
            ...
            for(int i = 0; i< words.length; i++) {
                if(str.equals(words[i])) wordCount++;
            }
